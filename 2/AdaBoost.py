...
if os.path.exists(ada_classifier_model_path):
    ...
    ada_classifier.fit(train_features, np.argmax(train_labels, axis=1))  # Ensure labels are in the right format
    
    # Save the trained AdaBoost model
    joblib.dump(ada_classifier, ada_classifier_model_path)
    print("AdaBoost classifier model saved.")

# Evaluate the AdaBoost classifier model on validation data
