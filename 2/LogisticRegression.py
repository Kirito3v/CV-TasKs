...
lr_classifier_model_path = 'lr_classifier_model.pkl'
...
train_features, train_labels = extract_or_load_features(
    feature_extractor, train_data, "train_features.npy", "train_labels.npy"
)

# Use the function for validation data
validation_features, validation_labels = extract_or_load_features(
    feature_extractor, validation_data, "validation_features.npy", "validation_labels.npy"
)
...
if os.path.exists(lr_classifier_model_path):
    print("Loading pre-trained Logistic Regression classifier model...")
    lr_classifier = joblib.load(lr_classifier_model_path)
else:
    print("Training Logistic Regression classifier model...")
    lr_classifier = LogisticRegression(max_iter=1000)
    lr_classifier.fit(train_features, np.argmax(train_labels, axis=1))  # Ensure labels are in the right format
    
    # Save the trained Logistic Regression model
    joblib.dump(lr_classifier, lr_classifier_model_path)
    print("Logistic Regression classifier model saved.")

# Evaluate the Logistic Regression classifier model on validation data
y_pred = lr_classifier.predict(validation_features)
accuracy = accuracy_score(validation_labels, y_pred)
print(f"Logistic Regression classifier model accuracy : {accuracy * 100:.2f}%")
